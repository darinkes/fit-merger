...
| `internal/editing` | Trim (by distance) and privacy-zone filtering of the record ends |
...
| `internal/preview` | Downsampled polyline (route, elevation, HR/cadence/power) for the browser |
| `web/` | Static single-page UI (`index.html`) + service worker, served with the wasm build |
...
- **`Record`'s optional fields are pointers** (`*float64`, `*uint8`, …) so
  ...
  a `nil` altitude must never be treated as `0 m`. In the browser preview these
  become `NaN` until each channel's first known sample (so a chart starts where
  its data does), then carry forward across dropouts.
- **Trim / privacy editing re-runs the merge.** `internal/editing` only *removes*
  whole records from the ends; the wasm then re-runs `merge.Merge` on the single
  edited activity so distance is re-based from zero and every total recomputed by
  the same trusted path — never patch the summary or `Distance` field by hand.
...
- `pages.yml` — on push to `main`, build `web/fitmerge.wasm`, copy `wasm_exec.js`,
  **stamp the git version into `web/sw.js`** (see below), and publish `web/` to
  GitHub Pages. Everything runs client-side.

  The service worker (`web/sw.js`) names its cache `fitmerge-<version>`; the
  version placeholder is replaced at build time in **both** `pages.yml` and
  `Dockerfile.web`. This is what makes each release land in a fresh cache
  namespace (old caches are purged on activate), so a new HTML shell never runs
  against a stale cached wasm engine. The HTML/wasm/JS trio is served
  network-first for the same reason; keep that coupling if you touch `sw.js`.
